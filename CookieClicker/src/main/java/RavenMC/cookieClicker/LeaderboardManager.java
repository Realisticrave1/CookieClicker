...
public class LeaderboardManager {
    ...
    private void saveLeaderboardLocation() {
        if (hologramLocation != null) {
            ...
            // If location is null, clear the saved location
            leaderboardConfig.set("location", null);
        }
    ...
    /**
     * Removes the leaderboard hologram
     * @return true if successful, false if no hologram exists
     */
    public boolean removeLeaderboard() {
        if (!decentHologramsAvailable || hologramLocation == null) {
            return false;
        }
        ...
        try {
            ...
            saveLeaderboardLocation(); // Save null location
            return true;
        } catch (Exception e) {
    ...
    /**
     * Moves the leaderboard up or down by the specified amount
     * @param amount The amount to move (positive for up, negative for down)
     * @return true if successful, false if no hologram exists
     */
    public boolean moveLeaderboardVertical(double amount) {
        if (!decentHologramsAvailable || hologramLocation == null) {
            return false;
        }

        Location newLocation = hologramLocation.clone().add(0, amount, 0);
        return relocateLeaderboard(newLocation);
    }

    /**
     * Relocates the leaderboard to a new location
     * @param newLocation The new location
     * @return true if successful, false if no hologram exists
     */
    public boolean relocateLeaderboard(Location newLocation) {
        if (!decentHologramsAvailable) {
            return false;
        }
        ...
        try {
            ...
            // Create hologram at new location
            hologramLocation = newLocation;
            Object hologram = createHologramMethod.invoke(null, hologramId, newLocation);
            ...
            plugin.getLogger().severe("Error relocating leaderboard hologram: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    /**
     * Gets the current leaderboard location
     * @return The leaderboard location, or null if not set
     */
